...
const separators = infix_operators + parentheses + COMMA; 
    ...
    switch (action) {
      ...
      case Action.NEW_TOKEN_APPEND:
        ...
        if ((charClass === CharClass.QUOTE) || (charClass === CharClass.SEPARATOR)){
            ...
            // conserva blank prima di '-' per poter disambiguare meno unario
            if ((i > 0) && (fullInput[i-1] === BLANK) && (currentToken === '-')) {
              tokens.push(BLANK)
              console.log('logoTokenizerFSM push BLANK 1');
            }
          	tokens.push(currentToken);
          	// conserva blank dopo '-' per poter disambiguare meno unario
            if ((fullInput[i+1] === BLANK) && (currentToken === '-')) {
              tokens.push(BLANK)
              console.log('logoTokenizerFSM push BLANK 2');
            }
          }
          currentToken = '';
        }
      ...
      last_blank = true;
		if (token === '[') {
            ...
            if ((token === '-') &&
                ...
                (tokens.length >= i) && (tokens[i+1] != BLANK) && (!isNaN(tokens[i+1]))) {
              console.log('PARSE - meno unario');
              parsed.push({type: CellType.NUMBER, val: -Number(tokens[i+1])});
              i++;
              continue;  
            } else
